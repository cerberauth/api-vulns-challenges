...
This challenge demonstrates a JWT implementation that is vulnerable to null signatures.
...
```bash
# vulnerable: a token with an empty signature segment is treated as valid
go run main.go serve --vulnerable=true

# fixed: the signature is always verified, an empty signature is rejected
